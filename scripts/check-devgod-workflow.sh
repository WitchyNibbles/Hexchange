...
if (errors.length > 0) {
  console.error(`devgod workflow check failed: invalid coverage manifest artifact ${artifactPath}: ${errors.join("; ")}`);
  process.exit(1);
}
EOF
}
...
if (errors.length > 0) {
  console.error(`devgod workflow check failed: invalid progress proof artifact ${artifactPath}: ${errors.join("; ")}`);
  process.exit(1);
}
EOF
}
...
require_artifact_ref_path() {
  ...
  case "$key" in
    brief)
      expected_prefix=".devgod/work/briefs/"
      ;;
    plan)
      expected_prefix=".devgod/work/plans/"
      ;;
    task)
      expected_prefix=".devgod/work/tasks/"
      ;;
    reviewer|qa_engineer|security_reviewer)
      expected_prefix=".devgod/work/reviews/"
      ;;
    *)
...
if [[ "$brief_rel" == ".devgod/work/briefs/brief-${artifact_task_id}.md" ]]; then
...
if [[ -f "$plan_file" ]]; then
  ...
  if [[ "$plan_rel" == ".devgod/work/plans/plan-${artifact_task_id}.md" ]]; then
...
queue_file="$repo_root/.devgod/work/task-queue.json"
...
if (!target) {
  console.error(`devgod workflow check failed: task queue is missing current task "${taskId}"`);
  process.exit(1);
}

if (activeState === "active" && currentTaskId !== taskId) {
  console.error(
    `devgod workflow check failed: task queue current_task_id "${String(currentTaskId)}" does not match active task "${taskId}"`
  );
  process.exit(1);
}
...
for (const task of tasks) {
  if (!task || typeof task !== "object") {
    console.error("devgod workflow check failed: task queue contains a non-object task entry");
    process.exit(1);
  }
  ...
  for (const [field, label] of [
    ...
    if (normalized.length === 0) {
      console.error(
        `devgod workflow check failed: task queue task "${task.id}" must include at least one ${label}`
      );
      process.exit(1);
    }
  }
